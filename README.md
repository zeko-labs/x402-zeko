...
- `src/evm-escrow.js`: inspection helpers for shared or tenant-owned reserve-release escrow contracts
...
If you want provider failover for the hosted/self-hosted facilitator, supply a comma-separated list instead:

```bash
X402_BASE_RPC_URLS=https://mainnet.base.org,https://base-rpc.publicnode.com \
X402_EVM_RELAYER_PRIVATE_KEY=0x... \
pnpm evm:facilitator
```
...
For hosted/self-hosted Ethereum failover, `X402_ETHEREUM_RPC_URLS=url1,url2` is also supported.
