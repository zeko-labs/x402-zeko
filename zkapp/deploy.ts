...
async function waitForConfiguration(
  zkapp: X402SettlementContract,
  zkappAddress: PublicKey,
  beneficiary: PublicKey,
  serviceCommitment: ReturnType<typeof hashStringToField>,
  attempts = 40,
  intervalMs = 3000
) {
  for (let index = 0; index < attempts; index += 1) {
...
async function main() {
  ...
  const configureTx = await Mina.transaction(
    ...
    async () => {
      await zkapp.configure(
        new X402SettlementConfig({
          beneficiary,
          serviceCommitment
        })
      );
  ...
  const configured = await waitForConfiguration(
    zkapp,
    zkappAddress,
    beneficiary,
    serviceCommitment
