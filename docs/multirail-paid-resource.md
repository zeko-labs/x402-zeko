...
1. Base mainnet USDC first, because that matches the default Coinbase/Base/x402 mental model.
2. Ethereum mainnet USDC optionally, when you have a compatible hosted facilitator.
...
- If `X402_BASE_PAY_TO` or `X402_EVM_PAY_TO` is present, Base is included by default.
- If `X402_INCLUDE_ETHEREUM=true`, Ethereum mainnet is added too, using `X402_ETHEREUM_PAY_TO`, `X402_EVM_PAY_TO`, or falling back to the Base `payTo`.
