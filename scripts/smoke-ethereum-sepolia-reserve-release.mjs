...
function buildReserveReleaseRail({
  ...
  return buildEvmRail({
    ...
    settlementModel: "x402-ethereum-sepolia-usdc-reserve-release-v2",
    description: "Ethereum Sepolia reserve-release smoke path.",
...
function buildReserveReleaseIntent({
  ...
  return {
    primitive: "evm-ethereum-usdc-reserve-release-v2",
    settlementRail: "evm",
    network: {
      networkId: "eip155:11155111",
      chainId: 11155111,
...
async function ensureTokenAndEscrow({
  ...
  const configuredTokenAddress = readOptionalEnv("X402_ETHEREUM_SEPOLIA_TOKEN_ADDRESS");
  const configuredEscrowAddress = readOptionalEnv("X402_ETHEREUM_SEPOLIA_ESCROW_ADDRESS");
  const officialBalance = await publicClient.readContract({
    address: OFFICIAL_ETHEREUM_SEPOLIA_USDC,
    abi: ERC20_ABI,
    functionName: "balanceOf",
    args: [buyerAddress]
  });
  ...
  if (configuredTokenAddress && configuredEscrowAddress) {
    return {
      ...
      usingOfficialUsdc: configuredTokenAddress.toLowerCase() === OFFICIAL_ETHEREUM_SEPOLIA_USDC.toLowerCase(),
      deployed: []
    };
  }
  ...
  if (officialBalance >= amountAtomic && configuredEscrowAddress) {
    return {
      tokenAddress: OFFICIAL_ETHEREUM_SEPOLIA_USDC,
      escrowAddress: configuredEscrowAddress,
...
async function main() {
  const rpcUrl = readOptionalEnv("X402_ETHEREUM_SEPOLIA_RPC_URL", "https://ethereum-sepolia-rpc.publicnode.com");
  ...
  const relayer = privateKeyToAccount(
    normalizePrivateKey(
      readOptionalEnv("X402_ETHEREUM_RELAYER_PRIVATE_KEY", readOptionalEnv("X402_EVM_RELAYER_PRIVATE_KEY")),
      "X402_ETHEREUM_RELAYER_PRIVATE_KEY or X402_EVM_RELAYER_PRIVATE_KEY"
    )
  );
  const payTo = readOptionalEnv("X402_ETHEREUM_PAY_TO", readOptionalEnv("X402_EVM_PAY_TO"));
  if (!payTo) {
    throw new Error("X402_ETHEREUM_PAY_TO or X402_EVM_PAY_TO is required.");
  }

  const amount = readOptionalEnv("X402_ETHEREUM_SEPOLIA_USDC_AMOUNT", "0.01");
  ...
  if (relayerEth === 0n) {
    throw new Error(`Relayer ${relayer.address} has 0 ETH on Ethereum Sepolia.`);
  }
  ...
  const paymentContext = {
    serviceId: "zeko-x402-ethereum-sepolia-reserve-smoke",
    sessionId,
    turnId,
    baseUrl: "https://ethereum-sepolia.local",
    proofBundleUrl: `https://ethereum-sepolia.local/proof/${sessionId}.json`,
    verifyUrl: `https://ethereum-sepolia.local/verify/${sessionId}`,
    description: "Ethereum Sepolia reserve-release smoke path.",
    rails: [rail]
  };
  ...
  const reserveTxHash = settlement.transactionHash ?? settlement.txHash ?? settlement.transaction;
...
main().catch((error) => {
  console.error("[zeko-x402:smoke-ethereum-sepolia-reserve-release] failed", error);
  process.exit(1);
});
